...
ess_survey <- svy_make_scale(ess_survey, depression_items,
  ...
  svy_make_scale(healthy_eating_items, "healthy_eating",
                 r_key = -1)  # r_key = -1 reverses the entire scale after creation

```

# Report correlations

You can use `svy_cor_matrix()` to calculate weighted correlations, and then use `report_cor_table()` to present them nicely.
...
You can add distribution plots to correlation tables for survey data. These distributions are properly weighted using `svyhist()` for discrete variables and `svysmooth()` for continuous variables, ensuring they reflect the population rather than just the sample composition.

```{r}
ess_survey %>% svy_cor_matrix(var_names = var_renames) %>%
  report_cor_table(add_distributions = TRUE, data = ess_survey)
```

The `plot_distributions()` function can also be used directly with survey data:

```{r}
plot_distributions(ess_survey, var_names = var_renames)
```

These plots properly account for survey weights, so the distributions shown represent population estimates rather than raw sample frequencies.
...
```{r}
# Returns Cohen's d in the 'd' column
