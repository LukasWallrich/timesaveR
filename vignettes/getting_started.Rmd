...
This vignette introduces some of the basic functions that quickly get you from a dataset to publication-ready outputs. It uses `tidyverse`-functions alongside `timesaveR` as that is the recommended workflow.
...
Let's create scales for health behaviours and depressive symptoms, each including some reverse coding. For that, scale items need to be included into a named character vector; if any items are to be reversed, they should be added into another character vector.
...
Next, we are often interested in descriptive statistics, variable distributions and correlations. As far as numerical variables are concerned, all of them can be reported in a pretty table with the `apa_cor_table()` function. That function needs a correlation matrix as its first argument, which can usually be created with the `cor_matrix()` function.
...
#If var_names are provided, only variables included in that are included in the correlation table
ess_health %>% cor_matrix(var_names = var_renames) %>% apa_cor_table()
```

Often, it is interesting to also include variable distributions into this initial data summary table. This can also be done with the `apa_cor_table()` function. By default, variables with 5 distinct values or fewer are shown with a histogram, while for those with more values, a density plot is provided - but this can be changed with the `plot_type` argument. 

```{r}
ess_health %>% cor_matrix(var_names = var_renames) %>%
  apa_cor_table(add_distributions = TRUE, data = ess_health)
```
...
Then run `cat_var_table()` to get a table with the distribution of the categories and their association with the outcome variable.
...
ess_data <- tidyr::drop_na(ess_data)
...
mod1 <- lm(depression ~ agea + gndr + health + cntry, ess_data)
mod2 <- lm_std(depression ~ agea + gndr + health + cntry, ess_data)


mod3 <- lm(depression ~ agea * gndr + eisced + health + cntry, ess_data)
mod4 <- lm_std(depression ~ agea * gndr + eisced + health + cntry, ess_data)
